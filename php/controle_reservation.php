...
$equipements = $pdo->query("
    SELECT DISTINCT equipement FROM (
        SELECT equipement1 AS equipement FROM salle
        UNION
        SELECT equipement2 FROM salle
        UNION
        SELECT equipement3 FROM salle
    ) AS all_equipements
...
if ($equipement) {
    $countStmtSalle = $pdo->prepare("
        SELECT COUNT(*) FROM salle 
        WHERE equipement1 = :eq OR equipement2 = :eq OR equipement3 = :eq
    ");
    $countStmtSalle->execute(['eq' => $equipement]);
    $totalSalle = $countStmtSalle->fetchColumn();

    $stmtSalle = $pdo->prepare("
        SELECT id_salle AS id, nom FROM salle 
        WHERE equipement1 = :eq OR equipement2 = :eq OR equipement3 = :eq
    ...
    $stmtSalle = $pdo->prepare("SELECT id_salle AS id, nom FROM salle LIMIT :offset, :parpage");
    $stmtSalle->bindValue(':offset', $offsetSalle, PDO::PARAM_INT);
    $stmtSalle->bindValue(':parpage', $parPageSalle, PDO::PARAM_INT);
    $stmtSalle->execute();
}
